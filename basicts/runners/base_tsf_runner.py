...
class BaseTimeSeriesForecastingRunner(BaseEpochRunner):
        ...
        - Model:
            - Args:
                ...
                - future_data (torch.Tensor or None): Future data with shape [B, L, N, C]. 
                  Can be None if there is no future-normalized data available.
    ...
    def forward(self, data: tuple, epoch: int = None, iter_num: int = None, train: bool = True, **kwargs) -> Dict:
        ...
        Args:
            data (Dict): A dictionary containing 'target' (future-normalized data) and 'inputs' (history data) (normalized by self.scaler).
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]):
        ...
        if 'dual_view_loss' in forward_return:
            loss = loss + forward_return["dual_view_loss"]
    ...
    def val_iters(self, epoch, iter_index: int, data: Union[torch.Tensor, Tuple]):
    ...
    def test(self, train_epoch: Optional[int] = None, save_metrics: bool = False, save_results: bool = False) -> Dict:
        ...
        Args:
            ...
            save_results (bool): Save the test results_1. Defaults to False.
        """
